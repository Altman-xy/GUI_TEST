...
def start_care():
    ...
    # 来个死循环，24小时关心她
    print('现在是%s，已经开始守护'%(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime())))
    while (True):
        ...
        # if ':30:01' or ':00:00' in now_time1:
        #     print('您已经守护她半小时了！！', now_time1)
        # else:
        #     continue
        ...
        if (now_time == say_good_morning):
            ...
            print("提醒女友早上起床:%s" % time.ctime(),'内容为：',message)
            ...
            send_message(message)
            print("提醒女友中午吃饭:%s" % time.ctime(),'内容为：',message)
            ...
            send_message(message)
            print("提醒女友晚上吃饭:%s" % time.ctime(),'内容为：',message)
        ...
        elif now_time == say_midnight :
            message = choice(str_list_midnight)
            ...
            send_message(message)
            print("深夜提醒:%s" % time.ctime(),'\n内容为：',message)

        elif now_time == say_morning :
            message = choice(str_list_greet)
            send_message(message)
            print("早安提醒：%s" % time.ctime(),'\n内容为：',message)
...
def resend():
    # 简陋型回复
    send_content = input('请输入要回复的内容：')
    my_girl_friend.send(send_content)
    print('你发送的是：', send_content,"\n",time.ctime()[:-8],"\n")


# def pri_tips():
#     now_time = time.strftime('%Y-%m-%d %H:%M:%S',time.localtime())
#     while ':30:' in now_time :
#         print('您已经守护她半小时了！！')
#     else:
#         pass
...
if __name__ == "__main__":
    ...
    say_midnight = cf.get("configuration", "say_midnight")
    say_morning = cf.get("configuration", "say_morning_greet")
    ...
    # 深夜问候（防止微信长时间不活动掉线）
    str_list_midnight = ''
    with open("./remind_sentence/sentence_midnight.txt", "r", encoding='UTF-8') as f:
        str_list_midnight = f.readlines()
    print(str_list_midnight)

    # 9点上班叫起床
    str_list_greet = ''
    with open("./remind_sentence/sentence_morning.txt", "r", encoding='UTF-8') as f :
        str_list_greet = f.readlines()
    print(str_list_greet)
...
def print_others(msg):
    ...
    #       "\n" % (msg.text,now_mod_rank), time.ctime()[:-8],"\n")

    print("来自女友的消息:%s" % (msg.text),"\n",time.ctime()[:-8],"\n")
    for i in range(5):
        resend()
